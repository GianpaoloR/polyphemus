 ...
 *
 * For a file upload, the user provides a filename _template that
...
static CGI_varlist *
read_multipart(CGI_varlist *v, const char *_template) {
    ...
    for (;;) {
        ...
        if (filename != 0) {
            ...
            out = 0;
            if (_template != 0 && *filename != 0) {
                if (localname == 0) {
                    localname = (char *) mymalloc(strlen(_template) + 1);
                }
                strcpy(localname, _template);
...
CGI_encode_url(const char *p, const char *keep) {
    ...
    out = (char *) mymalloc(urlcount(p, keep) + 1);
 ...
 * with mkstemp() and "_template".  If the _template is null or if
...
CGI_get_post(CGI_varlist *v, const char *_template) {
    ...
    if ((env = getenv("CONTENT_TYPE")) != 0 &&
        strncasecmp(env, content_type, strlen(content_type)) == 0 &&
 ...
 * File uploads are handled using "_template" (see CGI_get_post())
 */

CGI_varlist *
CGI_get_all(const char *_template) {
    CGI_varlist *v = 0;

    v = CGI_get_cookie(v);
    v = CGI_get_query(v);
    v = CGI_get_post(v, _template);
    return v;
}
...
CGI_decode_base64(const char *p, int *len) {
    ...
    out = (unsigned char *) mymalloc(3 + 3 * strlen(p) / 4);
...
CGI_encode_base64(const void *p, int len) {
    const unsigned char *in = (unsigned char *) p;
    char *out;
    int save = 0, nbits = 0;
    int i, k = 0;
...
CGI_decode_hex(const char *p, int *len) {
    ...
    out = (unsigned char *) mymalloc(n / 2 + 1);
    for (i = k = 0; i < n; i += 2) {
